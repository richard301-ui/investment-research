...
(() => {
  ...
  function renderCaseTable(type){
    ...
    el.innerHTML=items.map(x=>{
      const href=type==='equity'?`report.html?ticker=${encodeURIComponent(x.ticker)}`:`bonds.html#${x.id}`;
      return `<tr class="case-row-link" data-href="${href}" tabindex="0" role="link" aria-label="Open ${x.ticker} analysis report">
        <td><a class="security-link" href="${href}"><span class="security">${x.ticker}</span><span class="subtext">${x.name}</span></a></td>
    ...
    el.querySelectorAll('.case-row-link').forEach(row=>{
  ...
  function renderReport(){
    ...
    const report=D.reports?.[ticker];
    const security=D.universe.find(x=>x.ticker===ticker);
    const caseData=D.equityCases.find(x=>x.ticker===ticker);

    if(!report||!security||!caseData){
      document.title='Report not found — Richard Zhou';
      root.innerHTML=`<section class="report-missing"><div class="kicker"><b>404</b>Research report</div><h1>Report not found.</h1><p>The requested ticker is not in the active equity research set.</p><a class="report-back" href="equities.html">← Back to equity research</a></section>`;
      return;
    }

    document.title=`${ticker} Analysis — Richard Zhou`;
    const order=D.equityCases.map(x=>x.ticker);
    const i=order.indexOf(ticker);
    const prev=order[(i-1+order.length)%order.length];
    const next=order[(i+1)%order.length];
    const prevName=D.equityCases.find(x=>x.ticker===prev)?.name||prev;
    const nextName=D.equityCases.find(x=>x.ticker===next)?.name||next;
    const list=items=>items.map(x=>`<li>${x}</li>`).join('');

    root.innerHTML=`
      <section class="report-hero">
        <a class="report-back" href="equities.html">← Equity research</a>
        <div class="kicker"><b>R / ${String(i+1).padStart(2,'0')}</b>Analysis report / ${security.type}</div>
        <div class="report-hero-grid">
          <div><div class="report-symbol">${ticker}</div><h1>${security.name}</h1></div>
          <div class="report-intro"><p>${report.deck}</p><div class="report-tags"><span>${report.stance}</span><span>${caseData.lens}</span><span>${security.horizon}</span></div></div>
        </div>
      </section>

      <section class="report-snapshot">
        <article><span>Research question</span><strong>${caseData.question}</strong></article>
        <article><span>Reported evidence</span><strong>${security.reported}</strong></article>
        <article><span>Evidence date</span><strong>${security.asOf}</strong></article>
      </section>

      <section class="report-body">
        <div class="report-main">
          <div class="kicker"><b>01</b>Expectations gap</div>
          <div class="report-block"><span>What the market may be pricing</span><p>${report.marketView}</p></div>
          <div class="report-block"><span>Working thesis</span><p>${report.workingThesis}</p></div>

          <div class="kicker report-kicker"><b>02</b>Evidence ledger</div>
          <div class="evidence-grid">${report.evidence.map(x=>`<div>${x}</div>`).join('')}</div>

          <div class="kicker report-kicker"><b>03</b>Underwriting work</div>
          <ol class="report-list numbered">${list(report.focus)}</ol>

          <div class="kicker report-kicker"><b>04</b>Path & risk</div>
          <div class="path-grid">
            <div><span>Catalysts</span><ul class="report-list">${list(report.catalysts)}</ul></div>
            <div class="risk-panel"><span>Risks</span><ul class="report-list">${list(report.risks)}</ul></div>
          </div>

          <div class="falsifier"><span>Falsifier</span><p>${report.falsifier}</p></div>
        </div>

        <aside class="report-side">
          <div class="side-card sticky-card"><span>Valuation status</span><h2>Model—not verdict.</h2><p>${report.valuation}</p><a href="models.html#reverse">Open valuation laboratory ↗</a></div>
          <div class="side-card"><span>Primary source</span><p>${security.reported}</p><a href="${security.sourceUrl}" target="_blank" rel="noopener">Open source ↗</a></div>
          <div class="side-card"><span>Research rule</span><p>Reported facts, analyst estimates and personal underwriting remain separate. Update the evidence before revising the conclusion.</p></div>
        </aside>
      </section>

      <nav class="report-next" aria-label="Adjacent reports">
        <a href="report.html?ticker=${prev}"><span>Previous</span><strong>${prev} / ${prevName}</strong></a>
        <a class="right" href="report.html?ticker=${next}"><span>Next</span><strong>${next} / ${nextName}</strong></a>
      </nav>`;
  }
  ...
  function initUniverse3D(){
    ...
    function select(h){
      ...
      const reported=document.querySelector('#detail-reported');
      const asOf=document.querySelector('#detail-asof');
      const source=document.querySelector('#detail-source');
      if(reported)reported.textContent=h.reported||'';
      if(asOf)asOf.textContent=h.asOf||'';
      if(source){
        source.hidden=!h.sourceUrl;
        if(h.sourceUrl)source.href=h.sourceUrl;
      }
